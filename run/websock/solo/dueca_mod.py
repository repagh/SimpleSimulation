...
ws1_priority = dueca.PrioritySpec(4, 0)

ws2_priority = dueca.PrioritySpec(5, 0)
...
# websockets at 10 Hz
sock_timing = dueca.TimeSpec(0, 1000)
...
if this_node_id == ecs_node:
    mymods.append(
        dueca.Module(
            ...
            ('add_channel',
             ('controls',              # variable
              f'ControlInput://{entity}', # channel name
    ...
    # external data sending over websock/json
    mymods.append(
        dueca.Module(
            "web-sockets-server", "", ws1_priority).param(
                set_timing=sock_timing,
                check_timing=(2000, 9000, 100)).param(
                    ('info', ('motion', "BaseObjectMotion://{entity}")),
                    ('port', 8001),
                    ('http_port', 8000),
                    ('document_root', './html'),
                    ('add-mimetype', ('.html', 'text/html')),
                )
        )

    # and for websock/msgpack
    mymods.append(
        dueca.Module(
            "web-sockets-server-msgpack", "", ws2_priority).param(
                set_timing=sock_timing,
                check_timing=(2000, 9000, 100)).param(
                    ('info', ('motion', "ObjectMotion://{entity}")),
                    ('port', 8002))
        )            
